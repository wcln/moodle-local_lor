...
class item_form extends \moodleform {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'general', get_string('general_settings', 'local_lor'));

        // Name
        $mform->addElement('text', 'name', get_string('item_name', 'local_lor'));
        $mform->setType('name', PARAM_TEXT);
        $mform->addHelpButton('name', 'item_name', 'local_lor');
        $mform->addRule('name', get_string('required'), 'required');

        // Description
        $mform->addElement('editor', 'description', get_string('item_description', 'local_lor'));
        ...
        // Topics
        $mform->addElement('text', 'topics', get_string('item_topics', 'local_lor'));
        ...
        // Categories
        $mform->addElement('autocomplete', 'categories', get_string('item_categories', 'local_lor'), category::get_all_menu(), ['multiple' => true]);
        $mform->addHelpButton('categories', 'item_categories', 'local_lor');
        $mform->addRule('categories', get_string('required'), 'required');

        // Grades
        $mform->addElement('autocomplete', 'grades', get_string('item_grades', 'local_lor'), grade::get_all_menu(), ['multiple' => true]);
        $mform->addHelpButton('grades', 'item_grades', 'local_lor');

        // Contributors
        $mform->addElement('autocomplete', 'contributors', get_string('item_contributors', 'local_lor'), contributor::get_form_options(), ['multiple' => true]);
