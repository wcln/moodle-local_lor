...
function local_lor_update_item($id, $type, $title, $topics, $categories, $grades, $contributors, $link, $width, $height, $video_id, $book_id, &$form) {
    ...
    if (! is_null($video_id)) {
        ...
        $DB->execute('UPDATE {lor_content} SET image = ? WHERE id = ?', array(
    ...
    if ($type == 1) { // Game.
        ...
        if ($form->get_file_content('image') !== false) {
            ...
            $DB->execute('UPDATE {lor_content} SET image = ? WHERE id = ?', array(
                "$CFG->wwwroot/_LOR/lessons/preview_images/$id.png",
                $id,
            ));
        }
...
function local_lor_is_designer() {
    ...
    if ($designer_cohort_id) {
        ...
        return $DB->record_exists('cohort_members', array(
            'userid'   => $USER->id,
            'cohortid' => $designer_cohort_id->id,
        ));
