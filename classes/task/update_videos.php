    ...
    public function execute()
    {
        ...
        if (property_exists($response, 'items')
            ...
            foreach ($response->items as $video) {
                ...
                $already_added = $DB->get_record_sql(
                    'SELECT content, video_id FROM {lor_content_videos} WHERE video_id = ?',
                    array($video_id),
                    IGNORE_MISSING
                );
                    ...
                    if (property_exists($response, 'items')
                        ...
                        foreach ($categories as $category) {
                            ...
                            if (preg_match(
                                ...
                                mtrace(
                                    "Found a category for the video: '$category->name'"
                                );
                        ...
                        if ( ! is_null($category_to_add)) {
                            ...
                            preg_match_all(
                                '|<meta property="og\:video\:tag" content="(.+?)">|si',
                                $connect,
                                $tags,
                                PREG_SET_ORDER
                            );
                            ...
                            foreach ($tags as $tag) {
                                ...
                                if ( ! preg_match(
                                    "/(?i)WCLN|BCLN|math|unit.*|[0-9]+|western|canadian|learning|network|sawatzky/",
                                    $tag[1]
                                )
                                ) {
                            ...
                            $record->title = preg_replace(
                                '/^(?i)[B,W]CLN\s*-*\s*|OSBC\s*-*\s*|Math\s*-*\s*|Chemistry\s*-*\s*|Physics\s*-*\s*|English\s*-*\s*/',
                                '',
                                $title
                            );
                            ...
                            $DB->execute(
                                'INSERT INTO {lor_content_categories}(content, category) VALUES (?,?)',
                                array(
                                    $id,
                                    (int)$category_to_add,
                                )
                            ...
                            mtrace(
                                "Video category added to lor_content_categories table."
                            );
                            ...
                            foreach ($topics as $word) {
                                ...
                                $existing_record = $DB->get_record_sql(
                                    'SELECT name FROM {lor_topic} WHERE name=?',
                                    array($word)
                                );
                                if ($existing_record) {
                                    $DB->execute(
                                        'INSERT INTO {lor_content_topics}(content, topic) VALUES (?,?)',
                                        array(
                                            $id,
                                            $word,
                                        )
                                    ...
                                    $DB->execute(
                                        'INSERT INTO {lor_topic}(name) VALUES (?)',
                                        array($word)
                                    );
                                    $DB->execute(
                                        'INSERT INTO {lor_content_topics}(content, topic) VALUES (?,?)',
                            ...
                            $DB->execute(
                                'INSERT INTO {lor_content_videos}(content, video_id) VALUES (?, ?)',
                                array(
                                    $id,
                                    $video_id,
                                )
                            ...
                            mtrace(
                                "Video ID added to lor_content_videos table."
                            );
                        } else {
                        ...
                        mtrace(
                            "Video is not in a category playlist. Skipping."
                        );
                    }
                }
            }
